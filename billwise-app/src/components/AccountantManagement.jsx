...
export default function AccountantManagement({ onSelectInvoice }) {
  ...
  const handleDeleteStaff = async () => {
    ...
    try {
      await merchantApi.deleteStaff(confirmDeleteStaff.id);
      showNotification(`Accountant @${confirmDeleteStaff.username} has been permanently deleted.`);
      setConfirmDeleteStaff(null);
      if (selectedStaff?.id === confirmDeleteStaff.id) {
        setSelectedStaff(null);
      }
      await loadData(true);
    } catch (err) {
      showNotification(err.message || 'Failed to delete accountant account', true);
    } finally {
  ...
  return (
    <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {/* Delete Accountant Button */}
                        <button
                          onClick={() => setConfirmDeleteStaff(accountant)}
                          className="p-1.5 rounded-xl text-xs font-bold transition border bg-red-50 text-red-600 hover:bg-red-100 border-red-200"
                          title="Delete Accountant Account"
                        >
      ...
      {/* ======================================================== */}
      {/* MODAL 3: CONFIRM DELETE ACCOUNTANT MODAL                  */}
      {/* ======================================================== */}
      {confirmDeleteStaff && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-6 bg-red-600 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-extrabold">Delete Accountant Account</h2>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-slate-600 leading-relaxed">
                Are you sure you want to permanently delete the accountant account for{' '}
                <strong className="text-slate-900">{confirmDeleteStaff.fullName || confirmDeleteStaff.username}</strong> (<span className="font-mono text-red-600 font-bold">@{confirmDeleteStaff.username}</span>)?
              </p>
              ...
              <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 space-y-1">
                ...
                <p className="text-[11px] text-amber-800 leading-relaxed">
                  Their login access and credentials will be permanently removed. All historical invoices previously created or audited by this accountant will remain safely preserved in your GST records.
                </p>
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-bold shadow-md shadow-red-600/20 transition disabled:opacity-60 flex items-center gap-1.5 cursor-pointer"
                >
                  <Trash2 className="w-3.5 h-3.5" />
